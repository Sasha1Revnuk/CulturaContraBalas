...
$(document).ready(function () {
    if ($('#models').length) {
        ...
        let url = `${language}/admin-menu/api/events`;
        let data = {};
        let blocks = dataHelper.getDatatable(columns, url, data, true)
        dataHelper.rowReorder(blocks, `${language}/admin-menu/api/events/sorting`)
    }


    $('body').on('click', '.delete', function (e) {
        e.preventDefault()
        dataHelper.deleteItem(translates, `${language}/admin-menu/api/events/delete/${$(this).attr('data-model')}`, null)
    })
});
